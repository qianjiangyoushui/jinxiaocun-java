...
<body>
	...
	<form action="${wechatPath}greenhouses/updateGreenhouses.action" method="post" id="formid">
		<input type="hidden" name="guid" id="guid" value="${house.guid}"/>
		...
		<div class="weui-cells weui-cells_form" style="margin-top: 0px;">
			<div class="weui-cell">
				...
				<div class="weui-cell__bd">
					<input class="weui-input" type="text" placeholder="大棚名称" name="housename" id="housename" style="margin-top: 5px;margin-bottom: 5px;" value="${house.housename}" onkeyup="check();">
				</div>
			</div>
			...
			<div class="weui-cell">
				...
				<div class="weui-cell__bd">
					<input class="weui-input" type="number" placeholder="距园区距离" name="distance" id="distance" style="margin-top: 5px;margin-bottom: 5px;" value="${house.distance}">
				</div>
			</div>
			
			<div class="weui-cell">
				<div class="weui-cell__hd">
					<label class="weui-label">大棚面积(亩)</label>
				</div>
				<div class="weui-cell__bd">
					<input class="weui-input" type="number" placeholder="大棚面积(亩)" name="areas" id="areas" style="margin-top: 5px;margin-bottom: 5px;" value="${house.areas}">
				</div>
			</div>
			...
			<div class="weui-cell">
				...
				<div class="weui-cell__bd">
					<input class="weui-input" type="text" placeholder="大概描述" name="description" id="description" style="margin-top: 5px;margin-bottom: 5px;" value="${house.description}">
				</div>
			</div>
	...
	function uploadimg(src,date,type,desc){
		...
		$("#submitid").on("click",function(){
			...
			$.ajax({
				url : "${wechatPath}greenhouses/updateGreenhouses.action",
				type : "POST",
				async : false,
				dataType : 'json',
				data : {
					...
					imageid :JSON.stringify(imageids)
				},
				success : function(data) {
					$.hideLoading();
					if(data.msg=='ok'){
						$.confirm("修改成功", function() {
							window.location.href="${wechatPath}greenhouses/detail.action?guid="+$("#guid").val();
					  }, function() {
