...
<body>
	<div style="height: 50px; line-height: 50px; background: #f7f7f7; text-align: center; font-size: 16px; border-bottom: 1px solid #bdbbbc; position: relative">
		施肥记录
		<a href="${wechatPath}g2g3/detail.action?seedfileid=${manure.seedfileid}&operate=${sessionScope.operate}&type=${sessionScope.g2g3Type}" style="position: absolute; left: 10px; top: 8px">
			<img src="${wechatPath}icon/back.png" style="width: 20px" />
		</a>
		<c:if test="${sessionScope.user.depart.departid eq '5' ||sessionScope.user.depart.departid eq '8'}">
			<a href="${wechatPath}manure/add.action?seedfileid=${manure.seedfileid}" style="position: absolute; right: 27px; top: 0px;">
				<img src="${wechatPath}icon/add.png" style="width: 20px" />
			</a>
			<a href="${wechatPath}manure/add.action?seedfileid=${manure.seedfileid}" style="position: absolute; right: 10px; top: 15px; text-decoration: none; font-size: 12px; color: #333">新建记录</a>
		</c:if>
	</div>
...
<!-- 	分页加载	 -->
	<script type="text/javascript">
		...
		function getdataPage(){
			...
			var id = '${manure.seedfileid}';
			$.ajax({
			  url: "${wechatPath}manure/pageList.action",
			  type:"POST",
			  async: true,
			  dataType: 'json',
			  ...
			  success: function(data){
				  ...
				  for(var i=0;i<rows.length;i++){
					 ...
					 content += '<div class="bd"><div> <a class="weui-cell weui-cell_access" href="${wechatPath}manure/detail.action?guid='+rows[i].guid+'"><div class="weui-cell__bd">';
					 content += '<strong style="font-size: 16px;color: #000000;">'+da.format('yyyy年MM月dd日')  +'施肥</strong>';
					 content += '<p></p>';
					 content += '<span style="font-size: 12px;color: #666666">农药名称：'+rows[i].muck+'</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">使用量：'+rows[i].dosage+rows[i].dosageunit+'</span><br/>';
