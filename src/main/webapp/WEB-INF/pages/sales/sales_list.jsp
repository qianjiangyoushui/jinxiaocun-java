...
<body>
	<div style="height: 50px; line-height: 50px; background: #f7f7f7; text-align: center; font-size: 16px; border-bottom: 1px solid #bdbbbc; position: relative">
		...
		<a href="${wechatPath}sales/add.action?clientid=${clientid}" style="position: absolute; right: 27px; top: 0px;">
			<img src="${wechatPath}icon/add.png" style="width: 20px" />
		</a>
		<a href="${wechatPath}sales/add.action?clientid=${clientid}" style="position: absolute; right: 10px; top: 15px; text-decoration: none; font-size: 12px; color: #333">新建记录</a>
	</div>
	...
	$("#datetime-start").calendar();
	 $("#datetime-end").calendar();
		...
		function getdataPage(){
			...
			$.ajax({
			  url: "${wechatPath}sales/page.action",
			  type:"POST",
			  async: true,
			  dataType: 'json',
			  ...
			  success: function(data){
				  ...
				  for(var i=0;i<rows.length;i++){
					  ...
					  content+='<a class="weui-cell weui-cell_access" href="${wechatPath}sales/detail.action?clientid=${clientid}&salesid='+rows[i].guid+'">';
					  ...
					  if(clientid==null || clientid ==""){
						  content+='-'+rows[i].client.clientname;
					  }
