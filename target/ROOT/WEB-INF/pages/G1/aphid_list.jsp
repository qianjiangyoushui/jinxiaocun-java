...
<body>
	<div style="height: 50px; line-height: 50px; background: #f7f7f7; text-align: center; font-size: 16px; border-bottom: 1px solid #bdbbbc; position: relative">
		蚜虫检测记录
		<a href="${wechatPath}greenhouses/housedetail.action?guid=${aphid.seedfileid}" style="position: absolute; left: 10px; top: 8px">
			<img src="${wechatPath}icon/back.png" style="width: 20px" />
		</a>
		<c:if test="${sessionScope.user.depart.departid eq '5'||sessionScope.user.depart.departid eq '8'}">
			<a href="${wechatPath}aphid/add.action?seedfileid=${aphid.seedfileid}" style="position: absolute; right: 27px; top: 0px;">
				<img src="${wechatPath}icon/add.png" style="width: 20px" />
			</a>
			<a href="${wechatPath}aphid/add.action?seedfileid=${aphid.seedfileid}" style="position: absolute; right: 10px; top: 15px; text-decoration: none; font-size: 12px; color: #333">新建记录</a>
		</c:if>
	</div>
...
<!-- 	分页加载	 -->
	<script type="text/javascript">
		...
		function getdataPage(){
			...
			var id = '${aphid.seedfileid}';
			$.ajax({
			  url: "${wechatPath}aphid/pageList.action",
			  type:"POST",
			  async: true,
			  dataType: 'json',
			  ...
			  success: function(data){
				  ...
				  for(var i=0;i<rows.length;i++){
					  var da = new Date(rows[i].recorddate);
					 content += '<div style="height: 130px; margin-top: 5px;margin-bottom:8px;;border:1px solid  #F0F0F0;background-color: #FFFFFF ">';
					 ...
					 content += '<p></p>';
					 content += '<span style="font-size: 12px;color: #666666">诱蚜盘编号：'+rows[i].aphidcode+'</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">蚜虫数量：'+rows[i].amount+'</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">监测人：'+rows[i].investigator+'</span><br/>';
