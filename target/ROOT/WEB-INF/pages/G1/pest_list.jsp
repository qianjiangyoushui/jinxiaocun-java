...
<body>
	<div style="height: 50px; line-height: 50px; background: #f7f7f7; text-align: center; font-size: 16px; border-bottom: 1px solid #bdbbbc; position: relative">
		病虫害发生情况记录
		<a href="${wechatPath}greenhouses/housedetail.action?guid=${pest.seedfileid}" style="position: absolute; left: 10px; top: 8px">
			<img src="${wechatPath}icon/back.png" style="width: 20px" />
		</a>
		<c:if test="${sessionScope.user.depart.departid eq '5' || sessionScope.user.depart.departid eq '8'}">
			<a href="${wechatPath}pest/add.action?seedfileid=${pest.seedfileid}" style="position: absolute; right: 27px; top: 0px;">
				<img src="${wechatPath}icon/add.png" style="width: 20px" />
			</a>
			<a href="${wechatPath}pest/add.action?seedfileid=${pest.seedfileid}" style="position: absolute; right: 10px; top: 15px; text-decoration: none; font-size: 12px; color: #333">新建记录</a>
		</c:if>
	</div>
	...
	$("#datetime-start").calendar();
	 $("#datetime-end").calendar();
		...
		function getdataPage(){
			...
			var id = '${pest.seedfileid}';
			$.ajax({
			  url: "${wechatPath}pest/pageList.action",
			  type:"POST",
			  async: true,
			  dataType: 'json',
			  ...
			  success: function(data){
				  ...
				  for(var i=0;i<rows.length;i++){
					  var da = new Date(rows[i].happendate);
					 content += '<div style="height: 180px; margin-top: 5px;margin-bottom:8px;;border:1px solid  #F0F0F0;background-color: #FFFFFF ">';
					 content += '<div class="bd"><div> <a class="weui-cell weui-cell_access" href="${wechatPath}pest/detail.action?guid='+rows[i].guid+'"><div class="weui-cell__bd">';
					 content += '<strong style="font-size: 16px;color: #000000;">'+da.format('yyyy年MM月dd日')  +'</strong>';
					 content += '<p></p>';
					 content += '<span style="font-size: 12px;color: #666666">病虫害种类：'+rows[i].pesttype+'</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">位置编号：'+rows[i].placecode+'</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">发生率：'+rows[i].probability+'%</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">调查人：'+rows[i].principal+'</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">记录人：'+rows[i].name+'</span><br/>';
