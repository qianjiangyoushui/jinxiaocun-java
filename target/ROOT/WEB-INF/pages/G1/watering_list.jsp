...
<body>
	<div style="height: 50px; line-height: 50px; background: #f7f7f7; text-align: center; font-size: 16px; border-bottom: 1px solid #bdbbbc; position: relative">
		灌溉情况记录
		<a href="${wechatPath}greenhouses/housedetail.action?guid=${watering.seedfileid}" style="position: absolute; left: 10px; top: 8px">
			<img src="${wechatPath}icon/back.png" style="width: 20px" />
		</a>
		<c:if test="${sessionScope.user.depart.departid eq '5' ||sessionScope.user.depart.departid eq '8'}">
			<a href="${wechatPath}watering/add.action?seedfileid=${watering.seedfileid}" style="position: absolute; right: 27px; top: 0px;">
				<img src="${wechatPath}icon/add.png" style="width: 20px" />
			</a>
			<a href="${wechatPath}watering/add.action?seedfileid=${watering.seedfileid}" style="position: absolute; right: 10px; top: 15px; text-decoration: none; font-size: 12px; color: #333">新建记录</a>
		</c:if>
	</div>
...
<!-- 	分页加载	 -->
	<script type="text/javascript">
		...
		function getdataPage(){
			...
			var id = '${watering.seedfileid}';
			$.ajax({
			  url: "${wechatPath}watering/pageList.action",
			  type:"POST",
			  async: true,
			  dataType: 'json',
			  ...
			  success: function(data){
				  ...
				  for(var i=0;i<rows.length;i++){
					  var da = new Date(rows[i].irrigatedate);
					 content += '<div style="height: 160px; margin-top: 5px;margin-bottom:8px;;border:1px solid  #F0F0F0;background-color: #FFFFFF ">';
					 content += '<div class="bd"><div> <a class="weui-cell weui-cell_access" href="${wechatPath}watering/detail.action?guid='+rows[i].guid+'"><div class="weui-cell__bd">';
					 content += '<strong style="font-size: 16px;color: #000000;">'+da.format('yyyy年MM月dd日')  +'</strong>';
					 content += '<p></p>';
					 content += '<span style="font-size: 12px;color: #666666">灌溉水量(L)：'+rows[i].irrigatewater+'L</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">喷灌时长(分钟)：'+rows[i].irrigatelength+'分钟</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">操作人：'+rows[i].recordperson+'</span><br/>';
