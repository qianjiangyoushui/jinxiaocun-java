	...
	$("#datetime-start").calendar();
	 $("#datetime-end").calendar();
		...
		function getdataPage(){
			...
			var id = '${eppo.seedfileid}';
			$.ajax({
			  url: "${wechatPath}eppo/pageList.action",
			  type:"POST",
			  async: true,
			  dataType: 'json',
			  ...
			  success: function(data){
				  ...
				  for(var i=0;i<rows.length;i++){
					  var da = new Date(rows[i].dodate);
					 content += '<div style="height: 180px; margin-top: 5px;margin-bottom:8px;;border:1px solid  #F0F0F0;background-color: #FFFFFF ">';
					 content += '<div class="bd"><div> <a class="weui-cell weui-cell_access" href="${wechatPath}eppo/detail.action?guid='+rows[i].guid+'"><div class="weui-cell__bd">';
					 content += '<strong style="font-size: 16px;color: #000000;">'+da.format('yyyy年MM月dd日')  +'</strong>';
					 content += '<p></p>';
					 content += '<span style="font-size: 12px;color: #666666">用途：'+rows[i].usetype+'</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">用药种类：'+rows[i].drugkind+'</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">用药总量：'+rows[i].drugdose+rows[i].drugdoseunit+'</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">负责人：'+rows[i].uploadperson+'</span><br/>';
					 content += '<span style="font-size: 12px;color: #666666">操作人：'+rows[i].principal+'</span><br/>';
