...
<body>
	<div style="height: 50px; line-height: 50px; background: #f7f7f7; text-align: center; font-size: 16px; border-bottom: 1px solid #bdbbbc; position: relative">
		灌溉情况记录
		<a href="${wechatPath}greenhouses/housedetail.action?guid=${irrigation.seedfileid}" style="position: absolute; left: 10px; top: 8px">
			<img src="${wechatPath}icon/back.png" style="width: 20px" />
		</a>
			<a href="${wechatPath}irrigation/add.action?seedfileid=${irrigation.seedfileid}" style="position: absolute; right: 27px; top: 0px;">
				<img src="${wechatPath}icon/add.png" style="width: 20px" />
			</a>
			<a href="${wechatPath}irrigation/add.action?seedfileid=${irrigation.seedfileid}" style="position: absolute; right: 10px; top: 15px; text-decoration: none; font-size: 12px; color: #333">新建记录</a>
	</div>
...
<!-- 	分页加载	 -->
	<script type="text/javascript">
		...
		function getdataPage(){
			...
			var id = '${irrigation.seedfileid}';
			$.ajax({
			  url: "${wechatPath}irrigation/pageList.action",
			  type:"POST",
			  async: true,
			  dataType: 'json',
			  ...
			  success: function(data){
				  ...
				  for(var i=0;i<rows.length;i++){
					 ...
					 content += '<div class="bd"><div> <a class="weui-cell weui-cell_access" href="${wechatPath}irrigation/detail.action?guid='+rows[i].guid+'"><div class="weui-cell__bd">';
