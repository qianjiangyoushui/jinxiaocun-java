...
<body>
	<div style="height: 50px; line-height: 50px; background: #f7f7f7; text-align: center; font-size: 16px; border-bottom: 1px solid #bdbbbc; position: relative">
		${depot.depotcode }事故列表
		<a href="javascript:history.go(-1);" style="position: absolute; left: 10px; top: 8px">
			<img src="${wechatPath}icon/back.png" style="width: 20px" />
		</a>
		<a href="${wechatPath}accident/accident_add.action?guid=${depot.guid}" style="position: absolute; right: 17px; top: 0px;">
			<img src="${wechatPath}icon/add.png" style="width: 20px" />
		</a>
		<a href="${wechatPath}accident/accident_add.action?guid=${depot.guid}" style="position: absolute; right: 15px; top: 15px; text-decoration: none; font-size: 12px; color: #333">新建</a>
	</div>
...
// 	 $("#datetime-end").calendar();
		...
		function getdataPage(guid){
			...
			$.ajax({
			  url: "${wechatPath}accident/page.action",
			  type:"POST",
			  async: true,
			  dataType: 'json',
			  data: {
			  ...
			  success: function(data){
				 ...
				 // href="${wechatPath}outin/outin_index.action?guid='+rows[i].guid+'"
				  ...
				  for(var i=0;i<rows.length;i++){
					  ...
					  content+='<a class="weui-cell weui-cell_access" href="${wechatPath}accident/accident_detail.action?guid='+rows[i].guid+'">';
					  content+='<div class="weui-cell__bd" style="margin-top:10px;">';
					  content+='<p style="font-size:17px;color:#000000">';
					  content+=rows[i].reportdate;
