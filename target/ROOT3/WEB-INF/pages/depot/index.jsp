...
<body>
	<div style="height: 50px; line-height: 50px; background: #f7f7f7; text-align: center; font-size: 16px; border-bottom: 1px solid #bdbbbc; position: relative">
		...
		<a href="${wechatPath}depot/add.action" style="position: absolute; right: 27px; top: 0px;">
			<img src="${wechatPath}icon/add.png" style="width: 20px" />
		</a>
		<a href="${wechatPath}depot/add.action" style="position: absolute; right: 10px; top: 15px; text-decoration: none; font-size: 12px; color: #333">增加仓库</a>
	</div>
	...
	$("#datetime-start").calendar();
	 $("#datetime-end").calendar();
		...
		function getdataPage(){
			...
			$.ajax({
			  url: "${wechatPath}depot/page.action",
			  type:"POST",
			  async: true,
			  dataType: 'json',
			  data: {"pageNo":pageNo},
			  ...
			  success: function(data){
				  ...
				  for(var i=0;i<rows.length;i++){
					  content+='<a style="color:#000000;" href="${wechatPath}depot/depot_index.action?guid='+rows[i].guid+'">';
					  content+='<div style="height:77px;width: 100% ;border:solid 1px #E0E0E0; resize:none;"><table><tr><td>';
					  content+='<div style="margin-left: 8px;margin-top: 6px;"><img src="${wechatPath}icon/test.png" style="width: 77px;height: 64px;"/></div></td>';
					  content+='<td><div  style="font-size:16px;margin-left: 30px;">'+rows[i].depotcode+'</div></td>';
